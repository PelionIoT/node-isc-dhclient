...
__thread char parse_err_str[DHCLIENT_MAX_ERR_STR];
...
isc_result_t read_client_conf_v8 (char **err_str)
{
...
//					&top_level_config);
	...
	status = read_client_v8_buffer (threadConfig->config_options, threadConfig->config_options_len,
					(struct interface_info *)0,
					&top_level_config);

	if (status != ISC_R_SUCCESS) {
		*err_str = get_parser_error_str();
		return status;
		;
#ifdef LATER
...
// node-isc-dhclient
// instead of a file we have a memory buffer
int read_client_v8_buffer (char *buf, int buflen, struct interface_info *ip,
...
//		return uerr2isc (errno);
	...
	cfile = NULL;
	status = new_parse(&cfile, -1, buf, buflen, NULL, 0); // -1 indicates 'memory' config file
	if (status != ISC_R_SUCCESS || cfile == NULL)         // see common/conflex.c:~107
		return status;
